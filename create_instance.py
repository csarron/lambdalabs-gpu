...
def main(instance_type, ssh_key_name):
    sleep_time = 20
    login(token=os.getenv('LAMBDA_CLOUD_TOKEN'))

    while True:
        try:
            instance_id = create_instance(instance_type, ssh_key_names=ssh_key_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ssh_key_name", type=str, required=True, help="ssh key name to be used.")
    args = parser.parse_args()
    main(args.instance_type, args.ssh_key_name)
